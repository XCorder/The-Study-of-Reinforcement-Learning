#python3 script
#Shortcut Maze (Exercise 8.4, using Dyna-Q Plus variation):bonus solely in action selection
#author: Xiang Chao
...
def epsilon_greedy_variation(epsilon, Q, state, Model, time_step, kappa):
    if random.random() < epsilon:
        action = random.randint(0, 3)
    else:
        Q_variation = Q[state[0], state[1], :]
        Q_variation = Q_variation + kappa*np.sqrt(time_step - Model[state[0], state[1], :, 3])
        Q_max = np.max(Q_variation)
        candidate = np.where(Q_variation == Q_max)[0]
...
for i in range(1, STEP+1):
    ...
    if i==(STEP//2+1):
        Maze[3, 8] = 0
    action = epsilon_greedy_variation(epsilon, Q, state, Model, time_step, kappa)
...
#np.save('cumulative_reward_Dyna_Q_Plus_variation_n_200_kappa_0.0009.npy', cumulative_reward)
