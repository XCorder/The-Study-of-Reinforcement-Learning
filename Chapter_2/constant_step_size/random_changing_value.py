#python3 script
#constant step size method, the changing of values are random
#author: Xiang Chao
...
def OneRun_1(arm_num, eps, step_num):#using sample averages, alpha = 1/n
    ...
    for i in range(step_num):
        value = 3*np.random.normal(0, 1, arm_num)#mu=0, sigma=1;the value for each arm
...
plt.legend()
plt.savefig('rcv_Average_reward.png')
plt.show()
...
plt.legend()
plt.savefig('rcv_Rate_optimal.png')