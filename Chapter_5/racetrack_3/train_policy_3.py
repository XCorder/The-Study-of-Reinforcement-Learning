#python3 script
#Racetrack(Exercise 5.9): train and save an optimal policy,discounting-aware
#author: Xiang Chao
...
eps_mu = 0.2# behavior policy is the 'eps_mu'-soft of the current target policy 
p_v_dont_change = 0
gamma = 0.9#the discount for reward
MaxEpisode = 10000000#300000#the max number of episodes
...
while True:
    ...
    if episode.pop() == 'Terminal':
        ...
        while episode:
            ...
            Q[state+action] = Q[state+action] + (G - W*Q[state+action]) / C[state+action]
            ...
            if tuple(A_greedy) != action:
                ...
                W = (1-gamma) + gamma*W / MU_action
                G = (-1 * W) + gamma*G / MU_action
    else:
        cnt_episode = cnt_episode - 1
...
np.save('policy_3_track_1_eps_mu_'+str(eps_mu)+'_pvdontch_'+str(p_v_dont_change)+'_episode_'+str(MaxEpisode)+'_gamma_'+str(gamma)+'.npy', PI)
